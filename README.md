...
The fixture contains an LRU-favoring locality phase, a FIFO-favoring cyclic scan, and a hot set that fits entirely. For every window, the phase table shows both an isolated run that starts empty and a continuous run that carries live cache state forward. Phase one favors LRU by two hits, phase two favors FIFO by one, and retained state contributes two hits to both policies in phase two without changing that decision. Across the full trace, LRU wins by one hit but FIFO transfers about 64 MiB less data because it avoids a second miss on the 64 MiB object. OPT stays visible as a hit-optimal offline ceiling; it is not presented as a byte-cost-optimal policy.
...
That run should pass the independent engine cross-check and show one LRU phase, one FIFO phase, and one tie in both the isolated and continuous views. Phase-two carry-over adds two hits to each online policy without changing the FIFO decision. The byte view recommends FIFO for phase two's steady state and for the trace overall, freezing the difference between request-count and transfer-cost optimization.
